...
3. **Intro loader hold**: the white `#xyz-wl` loader waits for `__xyzReelReady` (admin order applied, max ~2.5s) before exiting - fast reveal once the hero list is correct (fixes refresh UI flash/stuck).

4. **Light video preload** (`/* xyz-video-preload */`): once the admin order is applied, one-time sets the first ~4 hero videos to `preload="auto"` (and `load()`s the next 3, never the playing one) so the opening slides are buffered and don't show a loading circle. IMPORTANT - do NOT (a) preload all 8 videos or (b) release videos at runtime with `preload="none"`+`load()`: (a) froze the tab (memory/decoder exhaustion) and (b) caused thumbnail-freeze/black-frame/jump-in. Later slides load natively as the slideshow reaches them.
