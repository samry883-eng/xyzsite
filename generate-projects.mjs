...
<head>
  ...
  <style>
    ...
    */
    ...
    /* Cinematic section transition layers (scroll-driven) */
    .pj-video-area,
    .pj-video-frame,
    .pj-bottom-dock,
    .pj-credits,
...
<script>
  ...
  const videoArea = document.querySelector('.pj-video-area');
  const videoFrame = document.querySelector('.pj-video-frame');
  const bottomDock = document.querySelector('.pj-bottom-dock');
  const titleEl = document.querySelector('.pj-cr-title');
  const subEl = document.querySelector('.pj-cr-sub');
  const colsEl = document.querySelector('.pj-cr-cols');
  const framesEl = document.querySelector('.pj-frames');
  ...
  // Cinematic section-slide transition (editorial, layered, configurable)
  const TRANSITION_CFG = {
    triggerDistanceVh: 0.95,
    stageLiftPx: 72,
    frameLiftPx: 96,
    dockLiftPx: 46,
    frameScaleDrop: 0.06,
  ...
  let ticking = false;
  function applyCinematicScroll() {
    const rect = credits.getBoundingClientRect();
    const trigger = window.innerHeight * TRANSITION_CFG.triggerDistanceVh;
    const raw = (trigger - rect.top) / trigger;
    const p = easeOutCubic(clamp01(raw));

    if (videoArea) {
      videoArea.style.transform = 'translate3d(0,' + (-TRANSITION_CFG.stageLiftPx * p).toFixed(2) + 'px,0)';
      videoArea.style.opacity = String(1 - TRANSITION_CFG.stageFade * p);
    }
    if (videoFrame) {
      const frameY = -TRANSITION_CFG.frameLiftPx * p;
      const frameScale = 1 - TRANSITION_CFG.frameScaleDrop * p;
      videoFrame.style.transform = 'translate3d(0,' + frameY.toFixed(2) + 'px,0) scale(' + frameScale.toFixed(4) + ')';
      videoFrame.style.filter = 'blur(' + (TRANSITION_CFG.stageBlurPx * p).toFixed(2) + 'px)';
    }
    if (bottomDock) {
      bottomDock.style.transform = 'translate3d(0,' + (-TRANSITION_CFG.dockLiftPx * p).toFixed(2) + 'px,0)';
      bottomDock.style.opacity = String(1 - (0.32 * p));
    }

    const creditsY = (1 - p) * TRANSITION_CFG.creditsRisePx;
    credits.style.transform = 'translate3d(0,' + creditsY.toFixed(2) + 'px,0)';
    credits.style.opacity = String(TRANSITION_CFG.creditsStartOpacity + (1 - TRANSITION_CFG.creditsStartOpacity) * p);
    credits.style.filter = 'blur(' + ((1 - p) * TRANSITION_CFG.creditsBlurPx).toFixed(2) + 'px)';

    const lag = (1 - p) * TRANSITION_CFG.textLagPx;
    if (titleEl) titleEl.style.transform = 'translate3d(0,' + lag.toFixed(2) + 'px,0)';
    if (subEl) subEl.style.transform = 'translate3d(0,' + (lag * 0.8).toFixed(2) + 'px,0)';
    if (colsEl) colsEl.style.transform = 'translate3d(0,' + (lag * 0.55).toFixed(2) + 'px,0)';
    if (framesEl) framesEl.style.transform = 'translate3d(0,' + (lag * 0.35).toFixed(2) + 'px,0)';
  }
