...
async function saveCatalog(catalog, redeploy = false) {
  ...
  if (!saved.ok) return { ok: false, redeployed: false, error: saved.error || 'Save failed' };
  let redeployed = false;
  if (redeploy) redeployed = await triggerProductionRedeploy();
  return { ok: true, redeployed, storage: saved.storage, error: undefined };
}
...
export async function handleProjectsCollection(req, res) {
  ...
  if (req.method === 'POST') {
    ...
    const { ok, redeployed, error } = await saveCatalog(catalog, body.redeploy !== false);
    json(res, ok ? 200 : 503, { ok, redeployed, catalog: ok ? catalog : undefined, error: ok ? undefined : error });
    return;
  }
  ...
  if (req.method === 'PUT') {
    ...
    const { ok, redeployed, error, storage } = await saveCatalog(incoming, body.redeploy !== false);
    json(res, ok ? 200 : 503, {
      ok,
      redeployed,
      storage,
...
export async function handleProjectById(req, res, id) {
  ...
  if (req.method === 'PUT') {
    ...
    const { ok, redeployed, error } = await saveCatalog(catalog, body.redeploy !== false);
    json(res, ok ? 200 : 503, { ok, redeployed, project: ok ? merged : undefined, error: ok ? undefined : error });
    return;
  }
  ...
  if (req.method === 'DELETE') {
    ...
    const { ok, redeployed, error } = await saveCatalog(catalog, body.redeploy !== false);
    json(res, ok ? 200 : 503, { ok, redeployed, error: ok ? undefined : error });
    return;
  }
...
export async function handleProjectsSeed(req, res) {
  ...
  const { ok, redeployed, error } = await saveCatalog(catalog, body.redeploy === true);
  json(res, ok ? 200 : 503, {
    ok,
    redeployed,
