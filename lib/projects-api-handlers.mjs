...
import { buildDefaultCatalog, mergeDefaultCatalogMissing } from './projects-default-catalog.mjs';
...
async function loadCatalog({ applyDefaults = false } = {}) {
...
export async function handleProjectsCollection(req, res) {
  if (req.method === 'GET') {
    const catalog = await loadCatalog({ applyDefaults: true });
...
export async function handleProjectsSyncDefault(req, res) {
  ...
  if (!added) {
    json(res, 200, { ok: true, added: 0, count: catalog.projects.length, redeployed: false });
    return;
  }
  const { ok, redeployed, error, warning } = await saveCatalog(catalog, body.redeploy === true);
  json(res, ok ? 200 : 503, {
...
export async function handleProjectsApi(req, res, urlPath) {
  ...
  const posterMatch = urlPath.match(/^\/api\/projects\/poster\/([^/]+)$/);
