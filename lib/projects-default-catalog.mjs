...
function applyDefaultCanonicalFields(p, def) {
  let repaired = 0;
  if (p.category !== def.category) { p.category = def.category; repaired++; }
  if (p.slug !== def.slug) { p.slug = def.slug; repaired++; }
  const canonicalHref = '/work/' + def.category + '/' + def.slug + '/';
  if (p.href !== canonicalHref) { p.href = canonicalHref; repaired++; }
  if (JSON.stringify(p.services || []) !== JSON.stringify(def.services || [])) {
...
export function mergeDefaultCatalogMissing(catalog) {
  ...
  catalog.projects.forEach((p) => {
    ...
    if (!def) return;
    const fallbackPoster = WORK_CDN + '69c0bbd38bf9e9ecfb7c293b_final%20test1.mp4_snapshot_00.27.567.jpg';
    if (def.poster && def.poster !== p.poster && (!p.poster || p.poster === fallbackPoster)) {
      p.poster = def.poster;
      repaired++;
    }
    repaired += applyDefaultCanonicalFields(p, def);
    if (projectHasCredits(p)) return;
