...
import { redisConfigured, redisGet, redisSet, redisConfigHelp } from './upstash-redis.mjs';
...
export async function getProjectsCatalog() {
  if (redisConfigured()) {
    const fromRedis = parseCatalog(await redisGet(REDIS_KEY));
...
export async function saveProjectsCatalog(catalog) {
  if (redisConfigured()) {
    const ok = await redisSet(REDIS_KEY, catalog);
    if (ok) return { ok: true, storage: 'redis' };
    if (process.env.VERCEL) {
      return {
        ok: false,
        error: redisConfigHelp(
          'Redis write failed (Capabilities admin may still save via Edge Config; Work CMS needs Redis for the ~22KB catalog).'
        ),
      };
    }
    // Local dev: bad Redis creds — fall through to file.
  } else if (process.env.VERCEL) {
    return {
      ok: false,
      error: redisConfigHelp(
        'Work CMS catalog is ~22KB — too large for Edge Config. Redis is required on Vercel (same credentials as Capabilities admin).'
      ),
    };
  }
  try {
