...
import { getRedis, redisConfigured, redisGet, redisSet, REDIS_CONFIG_HELP } from './upstash-redis.mjs';
...
export async function saveProjectsCatalog(catalog) {
  if (redisConfigured()) {
    ...
    if (process.env.VERCEL) {
      return {
        ok: false,
        error: `Redis write failed — ${REDIS_CONFIG_HELP}`,
      };
    }
    ...
    return {
      ok: false,
      error: `Work CMS catalog is ~22KB — too large for Edge Config on this plan. ${REDIS_CONFIG_HELP}`,
    };
  }
  try {
