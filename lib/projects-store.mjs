...
/** Stored catalog only — no default merge (list membership is whatever was saved). */
export async function getProjectsCatalogRaw() {
  return readCatalogFromStores();
}

/** Alias for getProjectsCatalogRaw. */
...
export function resolveProjectCredits(p, { applyDefaults = false } = {}) {
  ...
  if (typeof p.credits === 'string' && p.credits.trim()) {
    rows = p.credits
      .split('\n')
      .map((line) => {
...
export function projectHref(_category, slug) {
  return '/work/' + slug + '/';
}
...
export function normalizeProject(p, opts = {}) {
  ...
  return {
    ...
    projectType,
    featured: !!p.featured,
    // Draft = kept in the CMS, not published on the site.
    draft: !!p.draft,
    ...
    // Selected Work runs its own order; falls back to the main one until set.
    selectedOrder: Number.isFinite(Number(p.selectedOrder)) ? Number(p.selectedOrder) : undefined,
