...
import { getRedis, redisConfigured, redisConfigHelp } from './upstash-redis.mjs';
...
/** Same Redis write path as capabilities-auth saveStore(). */
async function writeCatalogToRedis(catalog) {
  ...
  try {
    ...
    console.error('[projects-store] Redis write failed:', detail);
...
export async function getProjectsCatalog() {
  ...
  if (r) {
    try {
      ...
      if (raw != null) {
        const fromRedis = parseCatalog(typeof raw === 'string' ? raw : JSON.stringify(raw));
