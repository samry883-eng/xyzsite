...
function parseAdminEmailAllowlist() {
  const raw = process.env.CAPABILITIES_ADMIN_EMAILS;
...
/**
 * Admin UI login: CAPABILITIES_ADMIN_SECRET is the password.
 * Optional CAPABILITIES_ADMIN_EMAILS=comma@list — if set, email must match.
 */
export function validateAdminCredentials(emailRaw, password) {
  const secret = process.env.CAPABILITIES_ADMIN_SECRET || '';
  if (!secret || password == null) return false;
  if (!timingSafeEqualPassword(password, secret)) return false;
