...
async function getRedis() {
  ...
  try {
    ...
    // Same REST client as Upstash; Vercel KV uses KV_REST_* (see Redis.fromEnv in @upstash/redis).
...
export async function saveStore(store) {
  ...
  if (process.env.VERCEL) {
    throw new Error(
      'Production writes need a Vercel database: Storage → create Upstash Redis or Vercel KV, link it to this project, redeploy, and ensure UPSTASH_REDIS_REST_URL + UPSTASH_REDIS_REST_TOKEN (or KV_REST_API_URL + KV_REST_API_TOKEN) are set. Or update CAPABILITIES_GRANTS_JSON in project settings and redeploy.'
    );
  }
  writeStore(store);
}
...
/** Normalize env pastes: trim, strip one pair of surrounding quotes from Vercel/UI. */
...
export function validateAdminPassword(password) {
  const secret = normalizeEnvSecret(process.env.CAPABILITIES_ADMIN_SECRET);
