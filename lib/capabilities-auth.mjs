...
export async function loadStore() {
  ...
  if (r) {
    try {
      ...
      redisDead = true; redisClient = undefined;
    }
  }
  if (ecReadable()) {
    const v = await getKey(EC_GRANTS_KEY);
    if (v && typeof v === 'object' && v.grants && typeof v.grants === 'object') return { grants: v.grants };
  }
  return readStore();
}
...
export async function saveStore(store) {
  ...
  if (ecWritable()) {
    const ok = await setKey(EC_GRANTS_KEY, { grants: store.grants });
    if (ok) return;
...
export async function canPersistGrants() {
  const r = await getRedis();
  return Boolean(r) || ecWritable() || !process.env.VERCEL;
}
