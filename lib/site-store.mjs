...
import { loadMkeyLookup, applyMkeyRow } from '../scripts/home-order-lib.mjs';
...
function normalizeWorkOrder(order, root = ROOT) {
  ...
  try {
    ...
    return {
      ...order,
      homeList: order.homeList.map((x) => applyMkeyRow(x, lookup)),
    };
  } catch (e) {
...
export async function getWorkOrderResolved(root = ROOT) {
  ...
  const baked = readBakedWorkOrder(root);
  return baked ? normalizeWorkOrder(baked, root) : null;
}
